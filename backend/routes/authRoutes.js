...
// Local Authentication Routes
// Replace localSignup/localLogin with signup/login
...
router.get('/login', (req, res, next) => {
  ...
  next();
}, authController.malLogin); // ✅ Fixed name

router.get('/callback', authController.malCallback); // ✅ Fixed name

// Google OAuth Routes
router.get('/google/login', googleAuthController.googleLogin);
router.get('/google/callback', googleAuthController.googleCallback);

// ✅ Session Debug Route for Testing OAuth Session
// router.get('/test-oauth', authController.testOAuthSession);
