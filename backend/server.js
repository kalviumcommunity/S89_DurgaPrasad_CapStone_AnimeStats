...
app.use(session({
  ...
  store: MongoStore.create({
    ...
    autoRemove: 'native',
    touchAfter: 24 * 3600,
    stringify: false, // Tell connect-mongo not to JSON.stringify
    serialize: (session) => session, // Store object directly
    unserialize: (session) => session // Retrieve object directly
  }),
  cookie: {
    secure: false,
    httpOnly: true,
